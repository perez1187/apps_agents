...
from .serializers import (PlayerAggregateResultsSerializer, ClubListMenuSerializer,
...
class ClubSummaryView(APIView,Pagination10000):
    ...
    def get(self, request, format=None):
        ...
        clubs = Clubs.objects.filter(
            ...
            Q(results_club__nickname_fk__player=request.user)
    ...
    def get(self, request, format=None):
        ...
        if (
            ...
            results = Reports.objects.filter(
                ...
                #     .values("agent_income")
                #     .filter(nickname_fk__player=request.user)
                # ),
...
class PlayerAggregateResultsView(APIView, Pagination10000):
    permission_classes = [IsAuthenticated] 
    ...
    def get(self, request, format=None):
        ...
        if (
                ...
                results = Results.objects.filter(
                    Q(nickname_fk__player__username=request.user),
        ...
        serializer = PlayerAggregateResultsSerializer(results, many=False)
